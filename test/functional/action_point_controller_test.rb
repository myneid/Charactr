...
class ActionPointsControllerTest < ActionController::TestCase
  ...
  def test_add_should_increment_user_action_points
    current_ap = characters(:flappy).current_action_points
    post :add, :character_id => characters(:flappy).id
    assert_equal  current_ap + 1, Character.find(characters(:flappy).id).current_action_points
  end
  
  def test_succesful_add_should_return_current_action_points
    Character.any_instance.expects(:current_action_points).at_least(1).returns(2)
    post :add, :character_id => characters(:flappy).id
  ...
  def test_succesful_remove_should_return_current_action_points
    Character.any_instance.expects(:current_action_points).at_least(1).returns(0)
    post :remove, :character_id => characters(:flappy).id
